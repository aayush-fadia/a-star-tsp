...
State::State(const int *adjacencyMatrix, const int size, const bool *visited, const int currentNode, const int cost,
        ...
        : adjacency_matrix(adjacencyMatrix), size(size), visited(visited), current_node(currentNode), cost(cost),
          prev(prev) {
    int heuristic = prims_mst(adjacencyMatrix, size, visited);
    if (heuristic == -1) {
        this->total_cost = -1;
    } else {
        this->total_cost = cost + prims_mst(adjacencyMatrix, size, visited);
        // f = g + h
    }
}
...
vector<State *> State::successors() {
    ...
    for (int dst = 0; dst < size; dst++) {
        int i = index(this->current_node, dst, this->size);
        if ((!this->visited[dst]) && (this->adjacency_matrix[i] != -1)) {
